...
export function AuthForm({ initialTab = "signin", onSuccess, bare = false }: AuthFormProps) {
  ...
  return (
    <div className={bare ? "w-full" : ""}>
      ...
      <div role="tablist" aria-label="Auth mode" className="inline-flex rounded-pill bg-brand-surface p-1">
        {(["signin", "signup"] as Tab[]).map((tt) => (
          <button
            ...
            className={`px-4 py-1.5 text-sm rounded-pill transition-colors ${
              tab === tt ? "bg-white text-brand-dark shadow-sm" : "text-brand-muted hover:text-brand-dark"
            }`}
          >
      ...
      <form className="mt-5 space-y-3" onSubmit={handleSubmit}>
        <div>
          <label htmlFor="auth-email" className="block text-xs font-medium text-brand-muted mb-1">
            {t("auth.emailLabel")}
          </label>
          <input
            id="auth-email"
        ...
        <div>
          <label htmlFor="auth-password" className="block text-xs font-medium text-brand-muted mb-1">
